...
#if defined(NDEBUG)
# error "Aiblockchain cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x12a95c866af41701eabb4b841d66353e39e2bac6d759304609ec9960c35ae5d0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aiblockchain: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aiblockchain Signed Message:\n";
...
{
    // Aiblockchain: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Aiblockchain: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.00347 * 24 * 60 * 60; // Aiblockchain: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Aiblockchain: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aiblockchain: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aiblockchain: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8cda56a424371703daf6fea45a552da5c473beb43052380549decee8f84ec55e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "AiBlockchain Keepitreal";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 3 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045eef7b435c9473ed9a20db38f43c4cffa13dbf0714926d778c80cdb8bc4a58628178bd65220e623680b2e71b2020512e9a73d5f4c34392058ab4e637141278b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1680881755;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084613609;

        if (fTestNet)
        {
            block.nTime    = 1680881735;
            block.nNonce   = 387297611;
        }
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str()); 
        ...
        assert(block.hashMerkleRoot == uint256("0xb3f43c7e336b85943197faf9b2df93a06768c59d7c45657ea7006a38428eee8b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb7, 0xda }; // Aiblockchain: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AiblockchainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AiblockchainMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AiblockchainMiner(CWallet *pwallet)
{
    printf("AiblockchainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aiblockchain-miner");
    ...
    try { loop {
        ...
        printf("Running AiblockchainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AiblockchainMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AiblockchainMiner, pwallet));
}
